...
## 문제 풀이 리뷰
문제에서 사용자는 최대로 고를 수 있는 폰켓몬은 n/2 이다.
...
문제를 해결하고 다른 분들의 풀이를 보니 HashSet을 이용하여 문제를 해결하였다. HashSet을 이용하면 Set 인터페이스는 중복을 허용하지 않기 때문에 그냥 통째로 넣고 최대값보다 높으면 최대값을 반환하고 적으면 HashSet의 사이즈를 리턴하면 간단하게 해결할 수 있을 것 같다.
